...
scatter_Qf = lambda z : - 0.45 * (1+z) + 1.54
log10_M0_Qf = lambda z : 9.71 + 0.78 * (1+z)
fraction_Qf = lambda mass, z : 0.5+0.5*erf( ( mass - log10_M0_Qf(z) ) / scatter_Qf(z) ) 
...
beta_z = lambda z : -0.57 *z + 1.43
alpha_z = lambda z : 6.32 *z - 16.26
mean_SFR_Q = lambda mass, z : mass * beta_z(z) + alpha_z(z)
scale_z = lambda z : -0.34 * z + 0.99
...
print('N QU, log SFR[:10]',len(log_sfr[QU]), log_sfr[QU][:10], time.time()-t0)
print('N SF, log SFR[:10]',len(log_sfr[SF]), log_sfr[SF][:10], time.time()-t0)

# Hard X-ray emission, after Aird et al. 2018
def galaxy_lx(redshift, mass, sfr):
  return 10**(28.81)*(1+redshift)**(3.9)*mass + 10**(39.5)*(1+redshift)**(0.67)*sfr**(0.86)
...
x_itp = n.hstack(( 0., total_number[total_number>0.1], total_number[total_number>0.1][-1]*10)) 
y_itp = n.hstack(( mrange[total_number>0.1][0], mrange[total_number>0.1]   , mrange[total_number>0.1][-1] ))
...
if make_figure:
	...
	fig_out = os.path.join(fig_dir, 'star_formation_rate_hist_'+baseName+'.png')
	...
	fig_out = os.path.join(fig_dir, 'star_formation_rate_vs_zr_'+baseName+'.png')
	...
	fig_out = os.path.join(fig_dir, 'star_formation_rate_vs_mass_'+baseName+'.png')
