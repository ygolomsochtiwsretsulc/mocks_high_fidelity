...
for HEALPIX_32_id in n.arange(healpy.nside2npix(8)):
    ...
    if len(sf.nonzero()[0]) > 0:
        hdu_cols = fits.ColDefs([
            ...
            ##
            # AGN properties
            , fits.Column(name="HALO_M500c", format='D', unit='log10(M/[M_sun])', array=n.log10(M500c[sf])), fits.Column(name="HALO_Mvir", format='D', unit='log10(M/[M_sun])', array=n.log10(Mvir[sf])), fits.Column(name="HALO_halo_id", format='K', unit='', array=halo_id[sf]), fits.Column(name="HALO_pid", format='K', unit='', array=halo_host_id[sf]), fits.Column(name="AGN_LX_soft", format='D', unit='log10(Luminosity/[erg/s] 0.5-2 keV)', array=LX_soft[sf]), fits.Column(name="AGN_FX_soft", format='D', unit='Flux/[erg/cm2/s] 0.5-2 keV', array=FX_soft_attenuated[sf]), fits.Column(name="AGN_LX_hard", format='D', unit='log10(Luminosity/[erg/s] 2-10 keV)', array=LX_hard[sf]), fits.Column(name="AGN_FX_hard", format='D', unit='Flux/[erg/cm2/s] 2-10 keV', array=FX_hard[sf]), fits.Column(name="AGN_SDSS_r_magnitude", format='D', unit='mag', array=SDSS_r_AB_attenuated[sf]), fits.Column(name="AGN_nH", format='D', unit='log10(nH/[cm-2])', array=logNH[sf]), fits.Column(name="AGN_random_number", format='D', unit='', array=random[sf]), fits.Column(name="AGN_type", format='D', unit='X/opt type: 11, 12, 21, 22', array=agn_type[sf])
