...
agn_catalog_list = n.array(glob.glob(os.path.join( os.environ[env],'cat_AGN_'+ftyp, '*.fit' )))
agn_catalog_list.sort()
galaxy_catalog_list = n.array(glob.glob(os.path.join( os.environ[env],'cat_GALAXY_'+ftyp, '*.fit' )))
...
for path_2_eRO_catalog, path_2_GAL_catalog in zip( agn_catalog_list, galaxy_catalog_list):
	...
	def get_lognlogs_replicas(fx, area):
		...
		N_out = n.cumsum(out[0][::-1])[::-1]#n.array([n.sum(out[0][ii:]) for ii in range(len(out[0])) ])
	...
	for id_z in n.arange(len(all_zs)):
		...
		baseName = str_healpix_id + '_'+str(n.round(zmin,2))+ '_z_'+str(n.round(zmax,2))
