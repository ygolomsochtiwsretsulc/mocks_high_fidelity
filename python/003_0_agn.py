...
path_2_hard_RF_obs_soft = os.path.join( os.environ['GIT_AGN_MOCK'], "data", "xray_k_correction", "fraction_observed_A15_RF_hard_Obs_soft_fscat_002.txt")
path_2_RF_obs_hard = os.path.join( os.environ['GIT_AGN_MOCK'], "data", "xray_k_correction", "fraction_observed_A15_RF_hard_Obs_hard_fscat_002.txt")
path_2_NH_attenuation = os.path.join( os.environ['GIT_AGN_MOCK'], "data", "xray_k_correction", 'gal_nh_ratio_relation_newg16.dat')
...
Ls_h = lambda z : 10**(44.84 - n.log10( ((1+2.0)/(1+z))**3.87 + ((1+2.0)/(1+z))**(-2.12) ) )
phi_h = lambda L, z  : kz_h(z) / ( (L/Ls_h(z))**0.48 + (L/Ls_h(z))**2.27)
scatter_z = lambda x : scatter_0 - 2*x/30.
...
t1 = time.time() 
# select bins with a number of AGN greater than 1 and smaller than 2x the total number of agn, we want to simulate
bin_selection = (N_obs_th>=0.5)&(N_obs_th<n_agn*2.)
# draw LX luminosities uniformly in each LX bin, the bins (dlogf = 0.05) are small enough for a uniform sampling
...
lx0 = 43.2
lxz = lambda z : lx0 + erf(z)*1.2
width = 0.6
thick_fraction_z = lambda z : 0.30 # + erf(z)*0.1
thin_fraction_max = lambda LXhard : 0.9 * (41/LXhard)**0.5
thin_fraction_z = lambda z: thick_fraction_z(z) + 0.01 + erf(z/4.)*0.4
fraction_ricci = lambda LXhard, z : thin_fraction_z(z)+( thin_fraction_max(LXhard)-thin_fraction_z(z) )*(0.5+0.5*erf(( -LXhard + lxz(z) ) / width)) 
...
print(n_agn, len(thick.nonzero()[0])*1./n_agn, len(obscured.nonzero()[0])*1./n_agn, len(thinest.nonzero()[0])*1./n_agn)
...
obscuration_itp_H_S = interp2d(obscuration_z_grid, obscuration_nh_grid, obscuration_fraction_obs_erosita)
...
obscuration_z_grid, obscuration_nh_grid, obscuration_fraction_obs_erosita = n.loadtxt(path_2_RF_obs_hard, unpack=True)
obscuration_itp_H_H = interp2d(obscuration_z_grid, obscuration_nh_grid, obscuration_fraction_obs_erosita)
...
fraction_22p21_merloni = lambda lx : (0.5+0.5*erf((-lx+44.)/0.9))*0.69+0.26
...
def compute_agn_type(z, lx, logNH, fbins=fbins, n_agn=n_agn):
	...
	for bin_low, bin_high, num_needed, nn_un in zip( fbins[:-1], fbins[1:], n_per_bin_12.astype('int'), nth ):
		if num_needed>0 and nn_un>0 : 
			frac_needed = num_needed*1./nn_un
			sel_12 = (sel_12) | ((lx>bin_low)&(lx<bin_high)&(nh_23)&(rd<frac_needed))
	t_12 = (nh_23) & (sel_12)
	# second the types 21
	# to be chosen in nun
	n_per_bin_21 = (-fr_thk+limit)*nall
	sel_21 = (n.ones(len(z))==0)
	for bin_low, bin_high, num_needed, nn_un in zip( fbins[:-1], fbins[1:], n_per_bin_21.astype('int'), nun ):
			if num_needed>0 and nn_un>0 : 
				frac_needed = num_needed*1./nn_un
				sel_21 = (sel_21) | ((lx>bin_low)&(lx<bin_high)&(nh_21)&(rd<frac_needed))
	t_21 = (nh_21) & (sel_21)
...
r_mean = lambda log_FX0520 : -2.*log_FX0520 - 7.
scatter_t1 = lambda n_agn_int :norm.rvs( loc = 0.0, scale = 1.0, size = n_agn_int)
...
nh_law = interp1d(n.hstack((-10.**25, 10**n.hstack((10., NH_DATA[0], 25)))), n.hstack((1., 1., 1./NH_DATA[1], 0.00001)))
...
ext_values = n.array([extinction.fitzpatrick99(n.array([6231.]), 3.1*EBV, r_v=3.1, unit='aa')[0] for EBV in ebv_values])
...
halo_data.create_dataset('FX_soft_attenuated', data = agn_rxay_flux_05_20_observed )
ds.attrs['units'] = 'F_X / [0.5-2keV, erg/cm2/s]'
...
if make_figure:
	...
	fig_out = os.path.join(fig_dir, 'AGN_SDSS_r_AB_attenuated_hist_'+baseName+'.png')
	X=AGN_SDSS_r_AB_attenuated

	p.figure(1, (6.,5.5))
	p.tight_layout()
	p.hist(X, histtype='step', rasterized=True, lw=4, bins=n.arange(12,30,1.0))
	p.title(baseName)
	...
	p.figure(1, (6.,5.5))
	p.tight_layout()
	p.hist(X, histtype='step', rasterized=True, lw=4, bins=n.arange(10,23,1.))
	p.title(baseName)
	p.xlabel('agn_type')
	p.ylabel('Counts')
	p.grid()
	...
	p.figure(1, (6.,5.5))
	p.tight_layout()
	p.hist(X, histtype='step', rasterized=True, lw=4, bins=n.arange(20,26.5,0.5))
	p.title(baseName)
	p.xlabel('logNH')
	p.ylabel('Counts')
	p.grid()
