...
for aexp_str in all_aexp[::-1]:
	...
	#zr_host = hdu_host[1].data['redshift_R']

	#sel_clusters = (zr_host >= zmin) & (zr_host < zmax)

	#zr_host_mean = n.mean(zr_host[sel_clusters])
	#print('zr_host_mean', zr_host_mean)
	#host_s135 = (hdu_host[1].data['HALO_M200c']>10**13.4) & (hdu_host[1].data['HALO_M200c']<10**13.6)
	#host_s14 = (hdu_host[1].data['HALO_M200c']>10**14.4) & (hdu_host[1].data['HALO_M200c']<10**14.6)
	#host_s145 = (hdu_host[1].data['HALO_M200c']>10**14.5)
	#area_Mpc2_1e135 = n.sum(n.pi * (hdu_host[1].data['HALO_Rvir'][sel_clusters & host_s135] / 1000)**2)
	#area_Mpc2_1e14 = n.sum(n.pi * (hdu_host[1].data['HALO_Rvir'][sel_clusters & host_s14] / 1000)**2)
	#area_Mpc2_1e145 = n.sum(n.pi * (hdu_host[1].data['HALO_Rvir'][sel_clusters & host_s145] / 1000)**2)
	#area_Mpc2 = n.sum(n.pi * (hdu_host[1].data['HALO_Rvir'][sel_clusters] / 1000)**2)
	#print('area_Mpc2', area_Mpc2)

	# SPIDERS DATA
	sel_spiders_host = (spiders_host['NMEMBERS'] >= 6) & (spiders_host['CLUZSPEC'] >= zmin) & (spiders_host['CLUZSPEC'] < zmax)
	...
	abs_mag_i_spiders = spiders_gal['CMODELMAG_I'][sel_spiders] - cosmo.distmod(spiders_gal['IDLSPEC1D_Z'][sel_spiders]).value
	weight = 1./frac_obs_itp(spiders_gal['CMODELMAG_I'][sel_spiders])
	...
	# figure of LF
	fig_out = os.path.join(fig_dir, 'cluster_galaxy_LF_z_'+aexp_str+'.png')
	mdex = 0.1
	...
	out = p.hist(mock_mag_abs[(x <= RADIUS)&(s14)], lw=2, weights=n.ones_like(mock_mag_abs[(x <= RADIUS)&(s14)]) / mdex / area_Mpc2_1e14 / n.log(10), bins = mbins, histtype = 'step', rasterized = True, label = r'$10^{14}<M_{200c}[M_\odot]<10^{14.2}$', color='black')
	#####
	#  HOST_HALO_M200c > 14.5
	#####
	s14 = (HOST_HALO_M200c>10**14.4) & (HOST_HALO_M200c<10**14.6)
	out = p.hist(mock_mag_abs[(x <= RADIUS)&(s14)], lw=2, weights=n.ones_like(mock_mag_abs[(x <= RADIUS)&(s14)]) / mdex / area_Mpc2_1e145 / n.log(10), bins = mbins, histtype = 'step', rasterized = True, label = r'$10^{14.4}<M_{200c}[M_\odot]<10^{14.6}$', color='red')
	#####
	#  full luminosity function
	#####
	s135 = (HOST_HALO_M200c>10**13.4) &  (HOST_HALO_M200c<10**13.6)
	out = p.hist(mock_mag_abs[(x <= RADIUS)&(s135)], lw=2, weights=n.ones_like(mock_mag_abs[(x <= RADIUS)&(s135)]) / mdex / area_Mpc2_1e135 / n.log(10), bins = mbins, histtype = 'step', rasterized = True, label = r'$10^{13.4}<M_{200c}[M_\odot]<10^{13.6}$', color='blue')
	out = p.hist(mock_mag_abs[is_quiescent & (x <= RADIUS)&(s135)], lw=2, weights=n.ones_like(mock_mag_abs[is_quiescent & (x <= RADIUS)&(s135)]) / mdex / area_Mpc2_1e135 / n.log(10), bins = mbins, histtype = 'step', rasterized = True, label = r'quiescent', color='blue', ls='dotted')
	out = p.hist(mock_mag_abs[(x <= 0.0001)&(s135)], lw=2, weights=n.ones_like(mock_mag_abs[(x <= 0.0001)&(s135)]) / mdex / area_Mpc2_1e135 / n.log(10), bins = mbins, histtype = 'step', rasterized = True, label = r'BCG', color='blue', ls='dashed')
