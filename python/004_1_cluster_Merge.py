...
path_2_CLU_files = n.array(glob.glob(os.path.join(test_dir, '*_CLU.h5')))
path_2_CLU_files.sort()

baseNames = n.array([ os.path.basename(path_2_CLU_file)[:-7] for path_2_CLU_file in path_2_CLU_files ])


def get_data(path_2_light_cone, path_2_coordinate_file, path_2_galaxy_file, path_2_CLU_file):
	...
	# Coordinates
	##
	fits.Column(name= "ra"             , format='D', unit='degree',        array = ra          )
	,fits.Column(name= "dec"           , format='D', unit='degree',         array = dec         )
	,fits.Column(name= "g_lat"         , format='D', unit='degree',         array = g_lat       )
	,fits.Column(name= "g_lon"         , format='D', unit='degree',         array = g_lon       )
	,fits.Column(name= "ecl_lat"       , format='D', unit='degree',         array = ecl_lat     )
	,fits.Column(name= "ecl_lon"       , format='D', unit='degree',         array = ecl_lon     )
	# distances
	,fits.Column(name= "redshift_R"    , format='D', unit='real space',     array = zzr         )
	,fits.Column(name= "redshift_S"    , format='D', unit='redshift space', array = zzs         )
	,fits.Column(name= "dL_cm"         , format='D', unit='cm',             array = dL_cm       )
	# extinction maps
	,fits.Column(name= "galactic_NH"   , format='D', unit='cm-2',        array = galactic_NH      )
	,fits.Column(name= "galactic_ebv"  , format='D', unit='mag',       array = galactic_ebv      )
	##
	# Galaxy properties
	##
	,fits.Column(name= "galaxy_stellar_mass"  , format='D', unit='log10(M/[M_sun])', array = galaxy_stellar_mass  )
	,fits.Column(name= "galaxy_star_formation_rate"  , format='D', unit='log10(SFR/[M_sun/year])', array = galaxy_star_formation_rate  )
	,fits.Column(name= "galaxy_LX_hard"  , format='D', unit='log10(LX (2-10keV)/[erg/s])', array = galaxy_LX_hard  )
	,fits.Column(name= "galaxy_mag_r", format='D', unit='mag', array = galaxy_mag_r  )
	,fits.Column(name= "galaxy_mag_abs_r", format='D', unit='mag err', array = galaxy_mag_abs_r  )
	##
	# AGN properties
	##
	#,fits.Column(name= "AGN_LX_soft"          , format='D', unit='Luminosity/[erg/s] 0.5-2 keV', array = agn_in_clu_LX_soft  )
	#,fits.Column(name= "AGN_FX_soft"          , format='D', unit='Flux/[erg/cm2/s] 0.5-2 keV',   array = agn_in_clu_FX_soft_attenuated    )
	...
	# Dark matter halo 
	##
	,fits.Column(name=  "HALO_M200c"            , format='D', unit='log10(M/[M_sun])',       array = HALO_cat["M200c"            ]/h )
	,fits.Column(name=  "HALO_M500c"            , format='D', unit='log10(M/[M_sun])',       array = HALO_cat["M500c"            ]/h )
	,fits.Column(name=  "HALO_Mvir"             , format='D', unit='log10(M/[M_sun])',       array = HALO_cat["Mvir"             ]/h )
	,fits.Column(name=  "HALO_Acc_Rate_1Tdyn"   , format='D', unit='Msun/yr',     array = HALO_cat["Acc_Rate_1_Tdyn"   ] )
	,fits.Column(name=  "HALO_b_to_a_500c"      , format='D', unit='axis ratio',            array = HALO_cat["b_to_a_500c"      ] )
	,fits.Column(name=  "HALO_c_to_a_500c"      , format='D', unit='axis ratio',            array = HALO_cat["c_to_a_500c"      ] )
	,fits.Column(name=  "HALO_rs"               , format='D', unit='kpc',         array = HALO_cat["rs"               ] )
	,fits.Column(name=  "HALO_rvir"             , format='D', unit='kpc',         array = HALO_cat["rvir"             ] )
	,fits.Column(name=  "HALO_vmax"             , format='D', unit='km/s',        array = HALO_cat["vmax"             ] )
	,fits.Column(name=  "x"             , format='D', unit='Mpc',        array = HALO_cat["x"] )
	,fits.Column(name=  "y"             , format='D', unit='Mpc',        array = HALO_cat["y"] )
	,fits.Column(name=  "z"             , format='D', unit='Mpc',        array = HALO_cat["z"] )
	#
	,fits.Column(name=  "halo_id"  , format='K', unit='', array = HALO_cat['id']               )
	#,fits.Column(name=  "pid"      , format='K', unit='', array = HALO_cat['pid'] )
	##
	# cluster properties
	##
	...
	,fits.Column(name='LX_soft_cin',         unit='log10(L_X/[0.5-2keV, cin, erg/s])' , format='D', array=LX_soft_cin         )
	,fits.Column(name='LX_soft_cex',         unit='log10(L_X/[0.5-2keV, cex, erg/s])' , format='D', array=LX_soft_cex         )
	,fits.Column(name='TX_cin',              unit='log10(T cin [keV])' , format='D', array=TX_cin              )
	,fits.Column(name='TX_cex',              unit='log10(T cex [keV])' , format='D', array=TX_cex              )
	,fits.Column(name='MICM_cex',            unit='log10(M_ICM [Msun]))' , format='D', array=MICM_cex            )
	,fits.Column(name='FX_soft',             unit='F_X / [0.5-2keV, erg/cm2/s]' , format='D', array=FX_soft             )
	,fits.Column(name='FX_soft_attenuated',  unit='F_X / [0.5-2keV, erg/cm2/s]' , format='D', array=FX_soft_attenuated  )
	,fits.Column(name='detected',            unit='boolean' , format='L', array=detected            )
	,fits.Column(name='scatter_1',           unit='random Gaussian scatter used loc=0, scale=1' , format='D', array=scatter_1           )
	,fits.Column(name='coolness',            unit='0:disturbed, 1:relaxed' , format='D', array=coolness            )
	])
...
for baseName in baseNames:
	...
	hdu_col = get_data(path_2_light_cone, path_2_coordinate_file, path_2_galaxy_file, path_2_CLU_file)
