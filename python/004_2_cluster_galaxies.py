...
itp_angular_size = interp1d( zzs, n.array([cosmo.kpc_comoving_per_arcmin(zz_i).value for zz_i in zzs ]) )
...
path_2_sat_galaxy_files     = n.array(glob.glob(os.path.join(test_dir, '*_galaxy.h5')        ))
path_2_sat_galaxy_files.sort()

baseNames = n.array([ os.path.basename(path_2_sat_galaxy_file)[:-10] for path_2_sat_galaxy_file in path_2_sat_galaxy_files ])
baseNames.sort()

# loop over all these files to create fit catalogs of galaxies around clusters in 2 rvir angular

def get_data(path_2_light_cone, path_2_coordinate_file, path_2_galaxy_file, N_rvir_angular=1.):
	"""
	parameters: 
	...
	idx_arrays, distances = Tree_obj_Haversine.query_radius( deg_to_rad*n.transpose([dec_CLU, ra_CLU ]), r=rsearch_rad, return_distance=True)
	...
	angular_distance_to_cluster = n.hstack((distances))/(search_rad_1_rvir_deg[CLU_ids]* n.pi/180.)
	r_o_rvir = ((x_coord_CLU[CLU_ids]-x[sf])**2.+(y_coord_CLU[CLU_ids]-y[sf])**2.+(z_coord_CLU[CLU_ids]-z[sf])**2.)**(0.5)/(rvir_CLU[CLU_ids]/1000.)
	...
	# Coordinates
	##
	fits.Column(name="ra"             , format='D', unit='degree',         array = ra       [sf[s_R]]   )
	,fits.Column(name= "dec"           , format='D', unit='degree',         array = dec      [sf[s_R]]   )
	,fits.Column(name= "g_lat"         , format='D', unit='degree',         array = g_lat    [sf[s_R]]   )
	,fits.Column(name= "g_lon"         , format='D', unit='degree',         array = g_lon    [sf[s_R]]   )
	,fits.Column(name= "ecl_lat"       , format='D', unit='degree',         array = ecl_lat  [sf[s_R]]   )
	,fits.Column(name= "ecl_lon"       , format='D', unit='degree',         array = ecl_lon  [sf[s_R]]   )
	# distances
	,fits.Column(name= "redshift_R"    , format='D', unit='real space',     array = zzr      [sf[s_R]]   )
	,fits.Column(name= "redshift_S"    , format='D', unit='redshift space', array = zzs      [sf[s_R]]   )
	,fits.Column(name= "dL_cm"         , format='D', unit='cm',             array = dL_cm    [sf[s_R]]   )
	# extinction maps
	,fits.Column(name= "galactic_NH"   , format='D', unit='cm',        array = galactic_NH   [sf[s_R]]   )
	,fits.Column(name= "galactic_ebv"  , format='D', unit='cm',       array = galactic_ebv   [sf[s_R]]   )
	##
	# Galaxy properties
	##
	,fits.Column(name= "galaxy_stellar_mass"  , format='D', unit='M_sun', array = galaxy_stellar_mass [sf[s_R]] )
	,fits.Column(name= "galaxy_star_formation_rate"  , format='D', unit='M_sun', array = galaxy_star_formation_rate [sf[s_R]] )
	,fits.Column(name= "galaxy_LX_hard"  , format='D', unit='M_sun', array = galaxy_LX_hard [sf[s_R]] )
	,fits.Column(name= "galaxy_mag_r", format='D', unit='', array = galaxy_mag_r [sf[s_R]] )
	,fits.Column(name= "galaxy_mag_abs_r", format='D', unit='', array = galaxy_mag_abs_r [sf[s_R]] )
	,fits.Column(name= "is_quiescent", format='L', unit='', array = is_quiescent [sf[s_R]] )
	##
	# Dark matter halo 
	##
	,fits.Column(name=  "HALO_M200c"            , format='D', unit='log10(M_sun)',array = f1[1].data["M200c" ][sf[s_R]]/h )
	,fits.Column(name=  "HALO_M500c"            , format='D', unit='log10(M_sun)',array = f1[1].data["M500c" ][sf[s_R]]/h )
	,fits.Column(name=  "HALO_Mvir"             , format='D', unit='log10(M_sun)',array = f1[1].data["Mvir"  ][sf[s_R]]/h )
	,fits.Column(name=  "HALO_Acc_Rate_1Tdyn"   , format='D', unit='Msun/yr',     array = f1[1].data["Acc_Rate_1_Tdyn"   ][sf[s_R]] )
	,fits.Column(name=  "HALO_rs"               , format='D', unit='kpc',         array = f1[1].data["rs"    ][sf[s_R]] )
	,fits.Column(name=  "HALO_rvir"             , format='D', unit='kpc',         array = f1[1].data["rvir"  ][sf[s_R]] )
	,fits.Column(name=  "HALO_vmax"             , format='D', unit='km/s',        array = f1[1].data["vmax"  ][sf[s_R]] )
	#,fits.Column(name=  "scale_of_last_MM"   , format='D', unit='',     array = f1[1].data["scale_of_last_MM"   ][sf[s_R]] )
	,fits.Column(name=  "Vrms"   , format='D', unit='km/s',     array = (f1[1].data["vx"][sf[s_R]]**2+f1[1].data["vy"][sf[s_R]]**2+f1[1].data["vz"][sf[s_R]]**2)**0.5 )   
	#
	,fits.Column(name=  "HALO_id"      , format='K', unit='', array = halo_id[sf[s_R]] )
	,fits.Column(name=  "HALO_host_id"     , format='K', unit='', array = CLU_ids[s_R] )
	,fits.Column(name=  "angular_distance_to_cluster_center_in_rvir" , format='D', unit='theta/theta_vir', array = angular_distance_to_cluster[s_R] )
	,fits.Column(name=  "comoving_distance_to_cluster_in_rvir"  , format='D', unit='r/r_vir', array = r_o_rvir[s_R] )
	,fits.Column(name=  "redshift_R_distance_to_cluster"     , format='D', unit='z_R-z_R_cluster', array = redshift_distance_R [s_R] )
	,fits.Column(name=  "redshift_S_distance_to_cluster"     , format='D', unit='z_S-z_S_cluster', array = redshift_distance_S [s_R] )
	])
	f1.close()
	return hdu_cols
...
for baseName in baseNames[::-1]:
	...
	path_2_out_file = os.path.join(tmp_dir, baseName+'_galaxiesAroundClusters.fit')

	hdu_col = get_data(path_2_light_cone, path_2_coordinate_file, path_2_galaxy_file)
