...
if make_figure:
	...
	fig_dir = os.path.join(os.environ['GIT_AGN_MOCK'], 'figures', env, 'coordinates' )
	...
	# DZ distribution
	fig_out = os.path.join(fig_dir, 'redshift_R-minus-redshift_S_hist_'+baseName+'.png')
	...
	p.figure(0, (8.,5.5))
	p.tight_layout()
	p.scatter(XX,YY,c=cc,s=3, marker='s', edgecolor='face', cmap='Paired', vmin=-3, vmax=2, rasterized=True)
	p.title(baseName)
	p.xlabel('R.A.')
	p.ylabel('Dec.')
	p.yscale('log')
	...
	p.figure(0, (8.,5.5))
	p.tight_layout()
	p.scatter(XX,YY,c=cc,s=3, marker='s', edgecolor='face', cmap='Paired', vmin=19.5, vmax=22, rasterized=True)
	p.title(baseName)
	p.xlabel('R.A.')
	p.ylabel('Dec.')
	p.grid()
