...
ug_RS_itp       = interp1d ( n.hstack(( 0., redshift_RS, 2.0)), n.hstack(( ug_RS[0], ug_RS, ug_RS[-1] )) )
gr_RS_itp       = interp1d ( n.hstack(( 0., redshift_RS, 2.0)), n.hstack(( gr_RS[0], gr_RS, gr_RS[-1] )) )
ri_RS_itp       = interp1d ( n.hstack(( 0., redshift_RS, 2.0)), n.hstack(( ri_RS[0], ri_RS, ri_RS[-1] )) )
iz_RS_itp       = interp1d ( n.hstack(( 0., redshift_RS, 2.0)), n.hstack(( iz_RS[0], iz_RS, iz_RS[-1] )) )

ug_sigma_RS_itp = interp1d ( n.hstack(( 0., redshift_RS, 2.0)), n.hstack(( ug_sigma_RS[0], ug_sigma_RS, ug_sigma_RS[-1] )) )
gr_sigma_RS_itp = interp1d ( n.hstack(( 0., redshift_RS, 2.0)), n.hstack(( gr_sigma_RS[0], gr_sigma_RS, gr_sigma_RS[-1] )) )
ri_sigma_RS_itp = interp1d ( n.hstack(( 0., redshift_RS, 2.0)), n.hstack(( ri_sigma_RS[0], ri_sigma_RS, ri_sigma_RS[-1] )) )
iz_sigma_RS_itp = interp1d ( n.hstack(( 0., redshift_RS, 2.0)), n.hstack(( iz_sigma_RS[0], iz_sigma_RS, iz_sigma_RS[-1] )) )

## ==============================
## Luminosity function model
## ==============================
# abundance matching for magnitudes based on Ricci et al. 2018. Tables 3,5, Eq. 8,9,10
logPhi_z, alpha_z,M_s_z = n.loadtxt(os.path.join(os.environ['GIT_AGN_MOCK'], 'data','cluster_galaxies','ricci_2018_table3.ascii'))

# redshift evolution function, Eq. 10
fun = lambda redshift, richness, a, b, c : a*n.log10(1+redshift) + b*n.log10(richness) + c
logPhi_evol = lambda redshift, richness : fun(redshift, richness, logPhi_z[0],logPhi_z[1],logPhi_z[2])
alpha_evol = lambda redshift, richness : fun(redshift, richness, alpha_z[0],alpha_z[1],alpha_z[2])
M_s_evol = lambda redshift, richness : fun(redshift, richness, M_s_z[0],M_s_z[1],M_s_z[2])

# schechter function, Eq. 8,9.
Schechter_L = lambda L, phi_s, L_s, alpha : phi_s * (L/L_s)**alpha * n.e**(-L/L_s) / L_s
Schechter_M = lambda M, phi_s, M_s, alpha : 0.4*n.log(10.)* phi_s * 10**(0.4*(M_s-M)*(alpha+1))  * n.e**(-10**(0.4*(M_s-M)))

Schechter_M_z = lambda M, redshift, richness : 0.4*n.log(10.)* 10**logPhi_evol(redshift, richness) * 10**(0.4*(M_s_evol(redshift, richness)-M)*(alpha_evol(redshift, richness)+1))  * n.e**(-10**(0.4*(M_s_evol(redshift, richness)-M)))
...
m_star_p1 = -22+2
bright_red_galaxies = (mag_abs_r_HAM_ini<=m_star_p1)&(is_quiescent)&(x<=1)
faint_galaxies = (~bright_red_galaxies)
# richness estimator, for red galaxies around the cluster such as m < m_star + 2
test3 = n.unique(hdu_clu[1].data['HALO_host_id'][bright_red_galaxies], return_counts=True, return_index=True, return_inverse=True)
...
richness_col = fits.Column( name='richness', format='K', unit='', array=richness)
...
for zmin, zmax in zip(z_bins,z_bins+0.1):
	...
	print('z_cluster_mean',z_cluster_mean, 'N clusters=', len(z_cluster[sel_clusters]))
	area_Mpc2 = n.sum(n.pi*(hdu_host[1].data['HALO_rvir'][sel_clusters]/1000)**2)
	...
	# scatter in the abundance matching relation: random Gaussian number, scale=0.15, loc=0
	rds = norm.rvs( loc=0, scale = 0.15, size = N_in_cluster )
	vmax_in_cluster = 10**(n.log10(vmax[sel]) + rds)
	mrange = n.arange(-30,-10,0.01)
	total_number = n.cumsum(Schechter_M_z(mrange, z_cluster_mean, richness_in_cluster) * area_Mpc2 * 0.01)
...
DM2 = DM2_itp(zr_CLU)
rmag_abs_ham_col = fits.Column( name='galaxy_mag_abs_r', format='D', unit='mag', array = mag_abs_r_HAM)
rmag_ham_col = fits.Column( name='galaxy_mag_r', format='D', unit='mag', array = mag_abs_r_HAM + DM2)
print('mean(initial magnitude - new magnitude)', n.mean(mag_abs_r_HAM_ini-mag_abs_r_HAM), time.time()-t0)
...
gr = gr_RS_itp(zr_CLU[is_quiescent]) + rds # + gr_sigma_RS_itp (zr_CLU[is_quiescent]) * rds / 10.
ri = ri_RS_itp(zr_CLU[is_quiescent]) + rds # + ri_sigma_RS_itp (zr_CLU[is_quiescent]) * rds / 10.
iz = iz_RS_itp(zr_CLU[is_quiescent]) + rds # + iz_sigma_RS_itp (zr_CLU[is_quiescent]) * rds / 10.
...
print('colors assigned', gr_all[:10], ri_all[:10], iz_all[:10], time.time()-t0)
...
depth_sdss_g = os.path.join(sdss_depth_dir, 'sdss_dr8_nodered_nside2048_g_model_10sigma.fits.gz')
depth_sdss_i = os.path.join(sdss_depth_dir, 'sdss_dr8_nodered_nside2048_i_model_10sigma.fits.gz')
depth_sdss_r = os.path.join(sdss_depth_dir, 'sdss_dr8_nodered_nside2048_r_model_10sigma.fits.gz')
depth_sdss_z = os.path.join(sdss_depth_dir, 'sdss_dr8_nodered_nside2048_z_model_10sigma.fits.gz')
#
NSIDE=2048
...
pixels = healpy.ang2pix(NSIDE, hdu_clu[1].data['ra'], hdu_clu[1].data['dec'], nest=False, lonlat=True)
...
print('SDSS magnitude errors computed', sdss_g_err[:10], sdss_r_err[:10], sdss_i_err[:10], sdss_z_err[:10], time.time()-t0)

# random variable in a norm distribution with sigma = 1

rds = norm.rvs( loc=0, scale = 1, size = N_gal  )
sdss_g_col = fits.Column( name='sdss_g', format='D', unit='mag', array = sdss_g_temp + rds * sdss_g_err )

rds = norm.rvs( loc=0, scale = 1, size = N_gal )
sdss_r_col = fits.Column( name='sdss_r', format='D', unit='mag', array = sdss_r_temp + rds * sdss_r_err )

rds = norm.rvs( loc=0, scale = 1, size = N_gal )
sdss_i_col = fits.Column( name='sdss_i', format='D', unit='mag', array = sdss_i_temp + rds * sdss_i_err )

rds = norm.rvs( loc=0, scale = 1, size = N_gal )
sdss_z_col = fits.Column( name='sdss_z', format='D', unit='mag', array = sdss_z_temp + rds * sdss_z_err )


sdss_g_err_col = fits.Column( name='sdss_g_err', format='D', unit='mag', array = sdss_g_err)
sdss_r_err_col = fits.Column( name='sdss_r_err', format='D', unit='mag', array = sdss_r_err)
sdss_i_err_col = fits.Column( name='sdss_i_err', format='D', unit='mag', array = sdss_i_err)
sdss_z_err_col = fits.Column( name='sdss_z_err', format='D', unit='mag', array = sdss_z_err)


print('creates hdu, time elapsed', time.time()-t0)

all_cols = []
for name, uni, forma in zip(hdu_clu[1].data.columns.names, hdu_clu[1].data.columns.units, hdu_clu[1].data.columns.formats):
	if name == "galaxy_mag_abs_r" or name == "galaxy_mag_r":
		print(name)
	else:
		all_cols.append(fits.Column(name=name, format=forma, unit=uni, array=hdu_clu[1].data[name]))
