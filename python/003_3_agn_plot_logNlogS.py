...
# Georgakakis 2008
path_2_logNlogS_data = os.path.join(os.environ["GIT_VS"], 'data', 'logNlogS', 'logNlogS_Georgakakis_08_AGN.data')
x_data, y_data = n.loadtxt(path_2_logNlogS_data, unpack=True)
p.plot(n.log10(x_data), n.log10(y_data), lw=3, ls='dotted', color='g' )

# Merloni 2012
path_2_logNlogS_data = os.path.join(os.environ["GIT_VS"], 'data', 'logNlogS', 'logNlogS_Merloni_12_AGN.data')
x_data, y_data  = n.loadtxt(path_2_logNlogS_data, unpack=True)
p.plot(n.log10(x_data), n.log10(y_data), lw=3, ls='dotted', color='r' )

# Mateos 2008
path_2_logNlogS_data = os.path.join(os.environ["GIT_VS"], 'data/logNlogS/logNlogS_Mateos_08_AGN.data')
...
for id_z in n.arange(len(all_zs)):
	...
	file_list = n.array(glob.glob(os.path.join(XLF_dir, 'XLF_soft_*_'+str(n.round(zmin,2))+ '_z_'+str(n.round(zmax,2))+'.ascii')))
	file_list.sort()
	data=[]
	for ff in file_list:
...
p.fill_between(xf,  y1=(nhar)*(1-(nharN)**(-0.5)), y2=(nhar)*(1+(nharN)**(-0.5)), color='g', alpha=0.7, label='Mock', lw=2)# 2-10  keV')
...
p.fill_between(xf,y1=1-(nharN)**(-0.5),y2= 1+(nharN)**(-0.5), color='green', alpha=0.3, label='mock ERR')
p.xlabel(r'$\log_{10}(L^{2-10\, keV}_X/[erg/s])$')
...
def get_lognlogr(mag_r, area):
	...
	x_out = 0.5*(out[1][1:] + out[1][:-1])
	N_out = n.cumsum(out[0]) # n.array([n.sum(out[0][ii:]) for ii in range(len(out[0])) ])
	c_out = N_out /area # n.array([n.sum(out[0][ii:]) for ii in range(len(out[0])) ]) / area
	c_out_up = (1 + N_out**(-0.5)) * c_out
	c_out_low = (1 - N_out**(-0.5)) * c_out
	c_err = (n.log10(c_out_up) - n.log10(c_out_low))/2.
	return x_out, c_out, c_err
...
n.savetxt(os.path.join(logNlogR_dir, 'logNlogR_optical_'+baseName+'.ascii'), DATA_R )
...
#p.xlim((9,12.5))
#p.ylim((1e-8,1e-1))
p.title('LX-stellar mass, '+str(n.round(zmin,2))+"<z<"+str(n.round(zmax,2)))
p.grid()
